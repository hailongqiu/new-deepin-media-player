...
class BottomControlPanel(object):
    ...
    def open_play_list_clicked(self, widget):
        ...
        if open_check == "True": # 隐藏播放列表
            if self.this.plugin_manage.plugins_dict.has_key("deepin_media_player_play_list_tree_view"):
                self.this.plugin_manage.plugins_dict["deepin_media_player_play_list_tree_view"].stop()
            self.ini.set("Window", "playlist", "False")                
            self.ini.save()
        else:    
            if self.this.plugin_manage.plugins_dict.has_key("deepin_media_player_play_list_tree_view"):
                self.this.plugin_manage.plugins_dict["deepin_media_player_play_list_tree_view"].start()                      
    ...
    def name(self):
        return "deepin-media-player-bottom-control-panel"
